...
whh = np.ones((4 * hidden_size, hidden_size + 1))
...
h, c, cache = flstm(whh)
dwxh, dwhh, dh_prev, dc_prev = step_backward_lstm(dout, cache, dc_prev, h, c)
...
affine: no problem
softmax:
* 1e-10. same numbers in numerical as in analytical
* bs, hs = 10 => 1e-8
